  ...
  return (
    <div className="mx-auto w-full max-w-4xl p-4 sm:p-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Buy Form */}
        <div className="rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20 sm:p-6">
          <div className="mb-6 flex flex-col items-start justify-between sm:flex-row sm:items-center">
            <h2 className="mb-2 text-lg font-semibold sm:mb-0">Buy {selectedCrypto.symbol}</h2>
          ...
          <Tabs
            value={buyOrderType}
            onChange={(value) => setBuyOrderType(value as string)}
            className="mb-6"
          >
            <Tabs.List grow>
          ...
          <div className="space-y-4">
            <div>
              ...
              <NumberInput
                ...
                className="w-full rounded-lg bg-gray-50 dark:bg-gray-600"
              />
            </div>
            <div>
            ...
            />
            <div>
              <label className="mb-2 block pt-2 text-sm font-medium">Total (USDT)</label>
              <NumberInput
                value={buyTotal}
            ...
            <button className="w-full rounded-lg bg-green-500 px-4 py-3 font-medium text-white transition-colors hover:bg-green-600">
              Buy {selectedCrypto.symbol}
            </button>
          </div>
        </div>
        {/* Sell Form */}
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20 sm:p-6">
          <div className="mb-6 flex flex-col items-start justify-between sm:flex-row sm:items-center">
            <h2 className="mb-2 text-lg font-semibold sm:mb-0">Sell {selectedCrypto.symbol}</h2>
            <div className="text-muted-foreground text-sm">
              Available:{' '}
              <span className="font-medium">
                {selectedCrypto.availableAmount.toFixed(8)} {selectedCrypto.symbol}
              </span>
            </div>
          </div>
          <Tabs
            value={sellOrderType}
            onChange={(value) => setSellOrderType(value as string)}
            className="mb-6"
          >
            <Tabs.List grow>
              <Tabs.Tab value="limit">Limit</Tabs.Tab>
          ...
          <div className="space-y-4">
            ...
            <button className="w-full rounded-lg bg-red-500 px-4 py-3 font-medium text-white transition-colors hover:bg-red-600">
      ...
      <div className="mt-4 text-center">
        <div className="text-muted-foreground bg-muted/50 inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm">
